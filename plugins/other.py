...
import plugins.others.gf2 as girlsfrontline2
try:
    from plugins.others.magnet import do_magnet_preview, parse_size
except ImportError:
    do_magnet_preview = None
    parse_size = None
try:
    from plugins.others.steam import source_server_info
except ImportError:
    source_server_info = None
from plugins.others import tarkov as Tarkov
from plugins.others.minecraft import MCRCON
from toogle.sql import SQLConnection
from toogle.utils import is_admin, modify_json_file
from configs import config, proxies
import plugins.others.racehorse as race_horse
try:
    import plugins.others.csgo as CSGO
except ImportError:
    CSGO = None
...
class CSGOBuff(MessageHandler):
    ...
    async def ret(self, message: MessagePack) -> MessageChain:
        if CSGO is None:
            return MessageChain.plain("CSGO 查询依赖 mysql-connector 未安装")
    ...
    async def ret(self, message: MessagePack) -> MessageChain:
        ...
        if content.startswith("regist"):
            ...
            if source_server_info is None:
                return MessageChain.plain("CS 服务器查询依赖 python-a2s 未安装")
            ...
            return MessageChain.plain("CSGO 库存依赖 mysql-connector 未安装")
...
class MarvelSnapZone(MessageHandler):
    ...
    def get_snap_card_data(self, name: str):
        ...
        info = {}
        for t in bs.find_all('div', {'class': 'item-name'}): 
            info[t.text] = t.parent.find('div', {'class': 'item-value'}).text.strip() # type: ignore
            
        for t in bs.find_all('div', {'class': 'name'}):
...
class ZLibDownload(MessageHandler):
    ...
    async def ret(self, message: MessagePack) -> Optional[MessageChain]:
        ...
        search_res = []
        for book in books.find_all('div', {'class': 'book-item'}): # type: ignore
            book_info = book.find('z-bookcard')
            book_title = book_info.find('div', {'slot': 'title'}).text # type: ignore
            book_author = book_info.find('div', {'slot': 'author'}).text # type: ignore
            
            search_res.append({
                'title': book_title,
                'author': book_author,
                'size': book_info.get('filesize'), # type: ignore
                'format': book_info.get('extension'), # type: ignore
                'language': book_info.get('language'), # type: ignore
                'year': book_info.get('year'), # type: ignore
                'rating': book_info.get('rating'), # type: ignore
                'quality': book_info.get('quality'), # type: ignore
                'publisher': book_info.get('publisher'), # type: ignore
                'download_link': f'{host}{book_info.get("download")}' # type: ignore
            })
            pass
