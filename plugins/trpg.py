...
try:
    from plugins.dnd.search_5e import search_magic
    from plugins.dnd.search_chm import search_chm
except Exception as e:
    search_magic = None
    search_chm = None

from plugins.dnd.DPRcalculator import get_dpr, draw_dpr

create_path('data/dice_table')

DND_IMAGES_PATH = Path("data/dnd5e/chm_jpg")
...
class Search5EMagic(MessageHandler):
    ...
    async def ret(self, message: MessagePack) -> MessageChain:
        if search_magic is None:
            return MessageChain.plain("DND5E 法术数据未配置")
...
class Search5ECHM(MessageHandler):
    ...
    async def ret(self, message: MessagePack) -> MessageChain:
        if search_chm is None:
            return MessageChain.plain("DND5E 天麟不全书数据未配置")
        msg = message.message.asDisplay()[5:].strip()
        res = search_chm(msg)
        if res.endswith("jpg"):
            res = Image.fromLocalFile(str(DND_IMAGES_PATH / res))
        else:
            res = Plain(res)
