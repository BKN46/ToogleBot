...
def save_anime_list(buffer_path: str = "data/anime/", ignore_cache: bool = False):
    ...
    for line in html.find_all('div'):
        if line.find('td', {'class': 'date2'}):
            now_weekday = line.find('td', {'class': 'date2'}).text # type: ignore
            continue
        if line.attrs.get('style') != "float:left":
            ...
            if detail_title:
                ...
                casts = [
                    x for x in
                    line.find('td', {'class': ['cast_r', 'cast_r1', 'cast_r2']}).text.split() # type: ignore
                    if x
                ]
                detail_map[detail_title] = {
                    'staff': detail_staff,
                    'casts': casts,
                    'type': line.find('td', {'class': ['type_c_r', 'type_b_r', 'type_a_r', 'type_d_r', 'type_e_r', 'type_f_r', 'type_g_r']}).text, # type: ignore
                    'tag': line.find('td', {'class': ['type_tag_r', 'type_tag_r1', 'type_tag_r2']}).text, # type: ignore
                }
            continue
        ...
        data = {
            ...
            'anime_img': info.find('img').attrs['data-src'], # type: ignore
            'anime_length': info.find_all('p')[1].text,
            'anime_start_time': info.find_all('p')[0].text,
            'anime_name': line.find('td').text, # type: ignore
        }
    ...
    font = ImageFont.truetype("tools/fonts/Arial Unicode MS Font.ttf", font_size)
    for anime in anime_list:
        ...
        anime_img = Image.open(io.BytesIO(requests.get(anime['anime_img'], stream=True, timeout=5).content))
