...
from toogle.message import Member

pic_temp_path = "data/luck_tmp.png"
font_path = "tools/fonts/AaRunXing.ttf"
...
def get_setu():
    ...
    # print(res.text)
    im = Image.open(io.BytesIO(requests.get(pic_url.replace(pixiv_proxy, pic_proxy), stream=True, timeout=5).content))
    return im
