...
def get_tarkov_goons():
    ...
    res = ["最近10次Goons目击:"]
    for find_card in bs.find_all('div', {"class": "card-body"})[1:11]:
        location = find_card.find('h5').text.strip() # type: ignore
        detailed_location = find_card.find_all('p')[1].text.strip()
        report_time = find_card.find_all('p')[2].script.text.strip() # type: ignore
        ...
        game_mode = find_card.find_all('p')[-1].text.strip()
