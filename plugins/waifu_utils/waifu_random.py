...
FONT_TYPE = "tools/fonts/Arial Unicode MS Font.ttf"
...
def get_designated_search(sexual, search_list, retry=3):
    ...
    if page_links:
        max_page = int(page_links.find_all("a", {"class": "flexitem pad"})[-1].text.strip()) # type: ignore
    ...
    chara_id = random.choice(chara_list).attrs["href"].split("?id=")[-1] # type: ignore
    while retry > 0:
        try:
...
def parse_anime_db(res):
    ...
    profile_src = (
        ...
        .split("?id=")[-1] # type: ignore
    )

    profile_vs_matches = soup.find(id="mainframe2").find_all(id="besttable") # type: ignore
    profile_vs_result = []
    # for vs in profile_vs_matches:
    #     against_chara_id = vs.find("td").find_all("a")[1].attrs["href"].split("?id=")[-1]
    #     against_chara_name = vs.find("td").find_all("a")[1].text
    #     vs_res = False
    #     if profile_id == against_chara_id:
    #         against_chara_id = (
    #             vs.find("td").find_all("a")[2].attrs["href"].split("?id=")[-1]
    #         )
    #         against_chara_name = vs.find("td").find_all("a")[2].text
    #         vs_res = True
...
def bgm_character_search(object_str):
    ...
    item_list = [
        ...
        for x in soup.find("div", {"id", "columnSearchB"}).find_all( # type: ignore
...
def buffered_url_pic(pic_url):
    ...
    PIL.Image.open(requests.get(pic_url, stream=True).content).save(buffer_path + trans_url)
    return PIL.Image.open(buffer_path + trans_url)
