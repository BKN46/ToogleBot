...
from plugins.compose.anime_calendar import save_anime_list
from plugins.compose.stock import get_search, get_stock_now, render_report
from plugins.others import baidu_index
from plugins.others.pcbench import get_compairison as get_pcbench_compairison
from plugins.others.weather import get_rainfall_graph
...
class AnimeDownloadSearch(MessageHandler):
    ...
    def search_anime(search_content: str):
        ...
        if not results:
            ...
            results = results.find_all('tr', {'class': ''})
        res = []
        for line in results:
            ...
            file_size = line.find_all('td')[4].text
            res.append({
                'title': title,
                'magnet': magnet,
                'url': url,
...
class FilmDownloadSearch(MessageHandler):
    ...
    async def ret(self, message: MessagePack) -> Optional[MessageChain]:
        ...
        soup = bs4.BeautifulSoup(res, "html.parser")
        search_res_raw = soup.find_all('li', {'class': 'media thread tap'})
