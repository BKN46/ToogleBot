...
from large_utils.femagic.langconv import Converter

DATA_PATH = "/root/repos/qqbot_graia/large_utils/femagic/data/"
...
def magic_stringify(data):
    try:
        desc = '\n'.join(data['entries'])
        duration = f"{data['duration'][0]['duration']['amount']} {data['duration'][0]['duration']['type']}" if data['duration'][0]['type'] != 'instant' else '立即'
        m_range = f"{data['range']['distance']['amount']} {data['range']['distance']['type']}" if data['range']['distance']['type'] != 'self' else '自身'
        upper_desc = '\n升环效果: ' + '\n'.join(data['entriesHigherLevel'][0]['entries']) if 'entriesHigherLevel' in data else ""
        return (
...
def search_magic(search_han):
    ...
    res = '\n'.join([f"{x[0]['name']} | {x[0]['level']}环 {x[0]['source']}" for x in res])
    return (
        f"没有匹配到具体法术，可能是:\n"
        f"{res}"
    )
